...
select 'Checking fields declared as FK without indexes...' as "Check name";
select
	...
	format(
		'CREATE INDEX ON %I.%I USING btree (%I)',
		n_source.nspname,
		c_source.relname,
		ar.attname
	)
...
join pg_attribute af on
	  af.attrelid = con.confrelid and af.attnum = any(con.confkey)
	  and not af.attisdropped
join pg_attribute ar on
	  ar.attrelid = con.conrelid and ar.attnum = any(con.conkey)
...
left join lateral (
		...
		join pg_attribute aidx on aidx.attrelid = i.indrelid
			and aidx.attnum = i.indkey[0] and not aidx.attisdropped
		where cr.oid = c_source.oid and aidx.attname = ar.attname
) t on true
WHERE
	contype = 'f'
	and (confupdtype <> 'a' or confdeltype <> 'a') -- "foreign" exclude "no action"
	and t.oid is null
